...
export default function TeamMemberCard({ member }) {
  return (
    <div className="group relative flex flex-col rounded-2xl overflow-hidden transition-all duration-300
      ...
      {/* 1. Top Banner */}
      <div className={`h-24 w-full bg-gradient-to-r ${member.bannerColor} opacity-90 relative p-4 flex justify-between items-start`}>
      ...
      <div className="px-4 py-4 flex-1 flex flex-col">
        ...
        {/* Name & Role */}
        <div className="mb-4">
          <h3 className="text-lg font-bold text-gray-800 dark:text-white leading-tight">{member.name}</h3>
          <p className="text-xs text-primary font-bold mt-1 uppercase tracking-wide">{member.role}</p>
        </div>

        {/* Details Grid */}
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div className="p-2 rounded-lg bg-gray-50 border border-gray-100 dark:bg-white/5 dark:border-white/5">
            <p className="text-[10px] uppercase text-gray-400 font-semibold tracking-wider">Student ID</p>
            <p className="text-xs font-mono text-gray-600 dark:text-gray-300 mt-0.5">{member.studentId}</p>
          </div>
          <div className="p-2 rounded-lg bg-gray-50 border border-gray-100 dark:bg-white/5 dark:border-white/5">
            <p className="text-[10px] uppercase text-gray-400 font-semibold tracking-wider">Department</p>
            <p className="text-xs font-medium text-gray-600 dark:text-gray-300 mt-0.5 truncate">{member.dept}</p>
          </div>
        </div>
